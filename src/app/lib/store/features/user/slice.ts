  ...
  reducers: {
    ...
    addCollectibles: (state, action: PayloadAction<BaseCollectible[]>) => {
      ...
      collectibles.forEach((collectible) => {
        ...
        const findFunction = (collectible: Collectible) =>
          collectible.icon === icon && collectible.quality === quality;
        const excludeFunction = (collectible: Collectible) =>
          !(collectible.icon === icon && collectible.quality === quality);

        const newCollectibleInCollectibles =
          state.collectibles.find(findFunction);
        ...
        if (!newCollectibleInCollectibles) {
          ...
          state.collectibles = [
            ...state.collectibles.filter(excludeFunction),
            {
              ...newCollectibleInCollectibles,
              count: newCollectibleInCollectibles.count + 1,
            },
          ];
        }
      });
    ...
    setCollectiblesToClaim: (
      state,
      action: PayloadAction<BaseCollectible[]>
    ) => {
